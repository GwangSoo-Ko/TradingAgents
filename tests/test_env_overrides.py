...
def test_reasoning_effort_defaults_to_none(monkeypatch):
    """Unset reasoning/thinking knobs keep their built-in default.

    openai/anthropic default to None (each provider uses its own default); this
    fork deliberately ships ``google_thinking_level="high"`` (fa3f6d2) so Gemini
    reasons at high effort by default.
    """
    dc = _reload_with_env(monkeypatch)

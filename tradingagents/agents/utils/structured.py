...
def invoke_structured_or_freetext(
    ...
    Returns ``(markdown, obj)``. ``obj`` is the typed result when the structured
    path succeeded, or ``None`` when the free-text fallback fired. Callers that
    only need the markdown discard the second element; the Portfolio Manager
    keeps it so the run can emit a machine-readable trade plan.
    """
